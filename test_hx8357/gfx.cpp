...
void GFX::begin() {
    ...
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_SWRESET);
    ...
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_SETEXC);
    digitalWrite(GPIO_HX8357D_DC, HIGH);
    SPI.transfer(0xFF);
    SPI.transfer(0x83);
    SPI.transfer(0x57);
    ...
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_SETOSC);
    digitalWrite(GPIO_HX8357D_DC, HIGH);
    SPI.transfer(0x6E);

    // Set panel to BGR
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_SETPANEL);
    digitalWrite(GPIO_HX8357D_DC, HIGH);
    SPI.transfer(0x05);

    // Set pixel format to 16 bit
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_COLMOD);
    digitalWrite(GPIO_HX8357D_DC, HIGH);
    SPI.transfer(0x55);

    // Sleep out
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_SLPOUT);
    digitalWrite(GPIO_HX8357D_DC, HIGH);
    delay(6); // Wait 5ms plus a safety margin

    // Display on
    digitalWrite(GPIO_HX8357D_DC, LOW);
    SPI.transfer(HX8357D_CMD_DISPON);
    digitalWrite(GPIO_HX8357D_DC, HIGH);
